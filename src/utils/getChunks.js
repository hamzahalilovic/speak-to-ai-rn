...
export const getData = async (
  entry,
  lastGoodAnswer,
  {scoreLimit, contentLng, sessionID},
  indexOption,
  debugOption = false,
) => {
  let response;
  ...
  try {
    ...
    const requestBody = {
      ...
      checkLanguage,
      contentLng,
      context: lastGoodAnswer?.answer,
      debug: debugOption,
    };
    response = await fetchWithTimeout(`${MIDDLEWARE_API_URL}detect`, {
      method: 'POST',
      timeout: 10000,
      headers: {
  ...
  try {
    const requestBody = {
      aggregate: searchAggregation,
      filter,
      docSource: 'S3',
      session: sessionID,
      question: searchEntry,
      topK: 10,
      scoreLimit: aggregate ? 0.1 : scoreLimit,
    ...
    if (!requestResponse.ok) {
      const err = Error(requestResponse.statusText, {
        cause: {
          ...
          info: await requestResponse.text(),
        },
      });
      return {error: err};
    }
  ...
  let functions = [];
  let tokens = 0;
  const sumOfScores = answer.result.reduce(
  ...
  console.log({sortResults, answer});
  if (sortResults && answer.result.length > 0) {
    ...
    response.result = answer.result.filter(
      d => d.createdAt.split('T')[0] === firstDate,
    );
    scores = answer.result.map(m => m.score);
  }
  ...
  return {
    ...
    followUp,
    entryType,
    newInput: searchEntry,
    langCode: detectedLng,
  };
};
